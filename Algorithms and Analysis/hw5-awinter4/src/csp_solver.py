...
def solve(n_meetings: int, date_range: set[datetime], constraints: set[DateConstraint]) -> Optional[list[datetime]]:
    ...
    def is_consistent() -> bool:
        for dc in constraints:
            if not dc.is_satisfied_by_assignment(cast(List[datetime], assignment)):
                return False
        return True
    ...
    found: bool = backtrack(0)
    
    if found:
        solution: List[datetime] = []
        for dt in assignment:
    ...
    for dc in constraints:
        if dc.arity() == 1:
            ...
            for d in domains[var]:
                if dc.is_satisfied_by_values(d):
                    valid.add(d)
            domains[var] = valid
...
def arc_consistency(domains: list[set[datetime]], constraints: set[DateConstraint]) -> None:
    ...
    Side Effects:
        ...
        directly within the provided domains parameter
    '''
    all_arcs: List[Tuple[int, int, DateConstraint]] = []
    for dc in constraints:
        if dc.arity() == 2:
            i: int = dc.L_VAL
            j: int = cast(int, dc.R_VAL)
            all_arcs.append((i, j, dc)) 
            all_arcs.append((j, i, dc))  

    queue = list(all_arcs)

    def revise(tail: int, head: int, dc: DateConstraint) -> bool:
        """
        Drop any value t from the tail’s domain that cannot match any 
        value h in the head’s domain under this constraint.
        Return True if removed at least one value.
        """
        removed: bool = False
        to_drop: list = []
        for t in domains[tail]:
            supported = False
            for h in domains[head]:
                if tail == dc.L_VAL and head == dc.R_VAL:
                    if dc.is_satisfied_by_values(t, h):
                        supported = True
                        break
                else:
                    if dc.is_satisfied_by_values(h, t):
                        supported = True
                        break
            if not supported:
                to_drop.append(t)
        for t in to_drop:
            domains[tail].remove(t)
            removed = True
        return removed

    while queue:
        tail, head, dc = queue.pop(0)
        if revise(tail, head, dc):
            if not domains[tail]:
                continue
            for (i, j, dc2) in all_arcs:
                if j == tail:
                    queue.append((i, j, dc2))