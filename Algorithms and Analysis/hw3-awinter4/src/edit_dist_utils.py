...
def get_edit_dist_table(row_str: str, col_str: str) -> list[list[int]]:
    ...
    '''
    rows, cols = len(row_str) + 1, len(col_str) + 1
    ...
    for i in range(1, rows):
        for j in range(1, cols):
            ...
            replace_cost = 0 if row_str[i - 1] == col_str[j - 1] else 1
            ...
            if (
                i > 1 and j > 1 and
                row_str[i - 1] == col_str[j - 2] and
                row_str[i - 2] == col_str[j - 1]
            ):
...
def get_transformation_list_with_table(s0: str, s1: str, table: list[list[int]]) -> list[str]:
    ...
    if i > 0 and j > 0 and table[i][j] == table[i - 1][j - 1] + (0 if s0[i - 1] == s1[j - 1] else 1):
        if s0[i - 1] == s1[j - 1]:
            return get_transformation_list_with_table(s0[:-1], s1[:-1], table)
        else:
            return ["R"] + get_transformation_list_with_table(s0[:-1], s1[:-1], table) 

    if i > 1 and j > 1 and s0[i - 1] == s1[j - 2] and s0[i - 2] == s1[j - 1] and table[i][j] == table[i - 2][j - 2] + 1:
        return ["T"] + get_transformation_list_with_table(s0[:-2], s1[:-2], table) 

    if j > 0 and table[i][j] == table[i][j - 1] + 1:
        return ["I"] + get_transformation_list_with_table(s0, s1[:-1], table) 

    if i > 0 and table[i][j] == table[i - 1][j] + 1:
        return ["D"] + get_transformation_list_with_table(s0[:-1], s1, table) 
