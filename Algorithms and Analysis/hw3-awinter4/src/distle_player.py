...
class DistlePlayer:
    ...
    def make_guess(self) -> str:
        ...
        '''
        sorted_words = sorted(self.possible_words, key=len)
        if sorted_words:
            return sorted_words[len(sorted_words) // 2]
        if self.all_words:
            return sorted(self.all_words, key=len)[len(self.all_words) // 2]
        return ""
    ...
    def get_feedback(self, guess: str, edit_distance: int, transforms: list[str]) -> None:
        ...
        for word in self.possible_words:
            table = get_edit_dist_table(guess, word)
            if table[len(guess)][len(word)] != edit_distance:
                continue

            if get_transformation_list_with_table(guess, word, table) == transforms:
